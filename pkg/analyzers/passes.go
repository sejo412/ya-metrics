...
/*
PassesAnalyzers returns set of standard go analyzers frm x/tools/go/analysis.
appends 			Package appends defines an Analyzer that detects if there is only one variable in append.
asmdecl 			Package asmdecl defines an Analyzer that reports mismatches between assembly files
and Go declarations.
...
buildssa 			Package buildssa defines an Analyzer that constructs the SSA representation of an error-free
...
errorsas 			The errorsas package defines an Analyzer that checks that the second argument to errors.
As is a pointer to a type implementing error.
fieldalignment 		Package fieldalignment defines an Analyzer that detects structs that would use less memory
...
framepointer 		Package framepointer defines an Analyzer that reports assembly code that clobbers
...
loopclosure 		Package loopclosure defines an Analyzer that checks for references to enclosing loop variables
from within nested functions.
lostcancel 			Package lostcancel defines an Analyzer that checks for failure to call a context
cancellation function.
nilfunc 			Package nilfunc defines an Analyzer that checks for useless comparisons against nil.
nilness 			Package nilness inspects the control-flow graph of an SSA function and reports errors
...
reflectvaluecompare Package reflectvaluecompare defines an Analyzer that checks for accidentally
...
sigchanyzer 		Package sigchanyzer defines an Analyzer that detects misuse of unbuffered signal as argument
to signal.Notify.
slog 				Package slog defines an Analyzer that checks for mismatched key-value pairs in log/slog calls.
sortslice 			Package sortslice defines an Analyzer that checks for calls to sort.Slice that do not use
a slice type as first argument.
...
stdversion 			Package stdversion reports uses of standard library symbols that are "too new" for the Go version
...
timeformat 			Package timeformat defines an Analyzer that checks for the use of time.Format or time.Parse
calls with a bad format.
unmarshal 			The unmarshal package defines an Analyzer that checks for passing non-pointer or non-interface t
ypes to unmarshal and decode functions.
...
waitgroup 			Package waitgroup defines an Analyzer that detects simple misuses of sync.WaitGroup.
*/
